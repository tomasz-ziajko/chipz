...
class Core {
        ...
        ticks_per_ms_(static_cast<uint64_t>(tick_frequency_hz_) / 1000u)
    {
        ...
        timer_.setElapsedCallback([this]() { pending_.store(true, std::memory_order_release); });
    }
    ...
    void onIRQ(IRQnType irqn)
    {
        ...
        if (idx < kRange) {
            isr_pending_[idx / 32u].fetch_or(1u << (idx % 32u), std::memory_order_release);
    ...
    void add(ChipBase& peripheral)
    {
        peripheral.setDeferCallbacks(
            [this, &peripheral](uint32_t ms) {
                uint64_t ticks = std::max(uint64_t(1), static_cast<uint64_t>(ms) * ticks_per_ms_);
                deferEntry(peripheral, ticks);
            },
            [this, &peripheral](uint32_t us) {
                uint64_t ticks = std::max(uint64_t(1), static_cast<uint64_t>(us) * tick_frequency_hz_ / 1000000u);
                deferEntry(peripheral, ticks);
            });
        ...
        for (int16_t irqn_raw : peripheral.requiredIRQs()) {
            const auto idx = static_cast<size_t>(static_cast<int>(irqn_raw) - static_cast<int>(kIRQnFirst));
            if (idx < kRange) {
                isr_table_[idx] = &peripheral;
            }
        }

        entries_.emplace_back(&peripheral, WaitCondition::immediate(), peripheral.run(),
    ...
    void service()
    {
        ...
        for (size_t w = 0; w < kMaskWords; ++w) {
            ...
            while (fired) {
                ...
                if (idx < kRange) {
                    const int16_t irqn_val = static_cast<int16_t>(static_cast<int>(idx) + static_cast<int>(kIRQnFirst));
                    for (auto& entry : entries_) {
                        if (entry.wait.type() == WaitCondition::Type::IRQ && entry.wait.irqn() == irqn_val) {
                            entry.wait = WaitCondition::immediate();
                        }
                    }
        ...
        for (auto& ce : comm_entries_) {
            if (ce.comm->hasInterruptPending()) {
                for (auto& entry : entries_) {
                    if (entry.wait.type() == WaitCondition::Type::Comm && entry.wait.commInterface() == ce.comm) {
                        entry.wait = WaitCondition::immediate();
                    }
                }
                if (ce.active_peripheral) {
                    ce.active_peripheral->onInterrupt(*ce.comm, ce.comm->getPendingInterruptType(),
    ...
    {
        return static_cast<size_t>(static_cast<int>(static_cast<int16_t>(irqn)) - static_cast<int>(kIRQnFirst));
    }
    ...
    WaitCondition resolveWaitCondition(const WaitCondition& cond) const
    {
        ...
        switch (cond.type()) {
            case WaitCondition::Type::DelayMs:
                return WaitCondition::deadline(now + static_cast<uint64_t>(cond.ms()) * ticks_per_ms_);
            case WaitCondition::Type::DelayUs:
                return WaitCondition::deadline(now + static_cast<uint64_t>(cond.us()) *
