...
struct type_index<T, First, Rest...> : std::integral_constant<size_t, 1 + type_index<T, Rest...>::value> {};
...
template <typename T, typename... Rest>
struct all_unique<T, Rest...> : std::bool_constant<!contains_v<T, Rest...> && all_unique<Rest...>::value> {};
...
class DriverTask {
    ...
    struct promise_type {
        ...
        DriverTask get_return_object() noexcept
        {
            return DriverTask{Handle::from_promise(*this)};
        }
    ...
    DriverTask(DriverTask&& o) noexcept : handle_(o.handle_)
    {
        o.handle_ = nullptr;
    }
    ...
    WaitCondition currentWait() const noexcept
    {
        return handle_.promise().wait_;
    }
 ...
 * and enables the static management methods (initializeAll, runAllMain,
 * etc.).
 *
 ...
 * Core calls getCommInterfaces() at add() time to register all of the
 * chip's communication interfaces. When an interrupt fires on any of them,
 * Core looks up which chip is active on that bus and calls onInterrupt() on
 * it, passing a reference to the specific interface that fired.
 *
 * SCHEDULING:
 ...
 * and is invoked by Chip<CommInterfaces...>::transmit() / receive()
 * wrappers before each transfer.
 */
class ChipBase {
    public:
    enum class Status {
    ...
    /**
     * @brief Legacy per-cycle entry point — override in pre-migration
     * drivers.
     *
     * New drivers should override run() instead. Core calls run(), whose
     * default implementation delegates here and returns
    ...
    /**
     * @brief Scheduling entry point — override as a coroutine returning
     * DriverTask.
     *
     ...
     * Can be overridden per driver class or at runtime via
     * Core::setPriority().
     *
     * @return Default priority (128 = mid-range)
     */
    virtual uint8_t getDefaultPriority() const
    {
        return 128;
    }
     ...
     *
     * Called by Core::add() to register each interface for interrupt
     * routing. Returns an empty span for chips with no communication
     * interfaces.
     *
     * @return Span over the chip's communication interfaces
     */
    virtual std::span<CommunicationInterface*> getCommInterfaces()
    {
        return {};
    }
     ...
     * dispatches to onTransferComplete / onRxComplete / onError /
     * onArbitrationLost.
     *
     ...
     */
    virtual void onInterrupt(CommunicationInterface& which, CommunicationInterface::InterruptType type, bool success)
    {
        (void)which;
        (void)type;
        (void)success;
     ...
     */
    virtual std::span<const int16_t> requiredIRQs() const
    {
        return {};
    }
     ...
     * requiredIRQs() fires. Runs in main-loop context — never in ISR
     * context. Cast irqn to the platform IRQn enum to identify the source.
     *
     * @param irqn Platform IRQ number (cast from port IRQn enum)
     */
    virtual void onIRQ(int16_t irqn)
    {
        (void)irqn;
    }
     ...
     */
    void setDeferCallbacks(std::function<void(uint32_t)> defer_ms, std::function<void(uint32_t)> defer_us)
    {
    ...
    /**
     * @brief Inject a claim-bus callback for a specific communication
     * interface
     *
     * Called by Core::add() once per registered interface. The Chip<>
     * template override stores the callback indexed by interface, and
     * invokes it before each transmit() / receive() to tell Core which chip
     * is active on that bus.
     *
     * @param comm Interface to associate the callback with
     * @param fn   Callback to invoke before each transfer on @p comm
     */
    virtual void setClaimBusCallback(CommunicationInterface* comm, std::function<void()> fn)
    {
        (void)comm;
        (void)fn;
    }
...
/**
 * @brief Template middle layer binding a chip to one or more communication
 * interfaces
 *
 ...
 * - getCommInterfaces()       — returns all interfaces for Core
 * registration
 * - Per-interrupt-type virtuals (onTransferComplete, onRxComplete, onError,
 *   onArbitrationLost) with the firing interface passed as the first
 * argument
 *
...
class Chip : public ChipBase {
    static_assert(detail::all_unique_v<CommInterfaces...>, "Chip: all communication interface types must be distinct");
    ...
    void onInterrupt(CommunicationInterface& which, CommunicationInterface::InterruptType type, bool success) override
    {
        using IT = CommunicationInterface::InterruptType;
        switch (type) {
            case IT::TransferComplete:
                onTransferComplete(which, success);
                break;
            case IT::RxComplete:
                onRxComplete(which);
                break;
            case IT::Error:
                onError(which);
                break;
            case IT::ArbitrationLost:
                onArbitrationLost(which);
                break;
        }
    }

    void setClaimBusCallback(CommunicationInterface* comm, std::function<void()> fn) override
    {
    ...
    protected:
    explicit Chip(CommInterfaces&... comms) : comms_{&comms...}
    {
     ...
     *
     * Call from initialize() with the ConnectionId returned by the
     * interface's registerConnection() method. From that point,
     * selectConnection() is called automatically before every transmit<T>()
     * and receive<T>() on that interface.
     *
    ...
    /**
     * @brief Async transmit with duration hint — arms CompletionSources on
     * the interface
     *
     * Identical to transmit<T>(data, length) in terms of bus claiming, but
     * forwards duration_us to the interface so CompletionSources can be
     * armed. On interfaces without CompletionSources this is equivalent to
     * the immediate overload.
     *
     * @tparam T          Interface type to transmit on
     * @param  data        Data to transmit
     * @param  length      Number of bytes
    ...
    virtual void onRxComplete(CommunicationInterface& which)
    {
        (void)which;
    }

    virtual void onError(CommunicationInterface& which)
    {
        (void)which;
    }

    virtual void onArbitrationLost(CommunicationInterface& which)
    {
        (void)which;
    }
