...
public:
    ...
    bool reset()      override { return reset_result; }
    bool isReady() const override { return ready; }
    ChipBase::Status getStatus() const override { return status; }
    std::string getDeviceId() const override { return "TestChip"; }
    bool main() override { return main_result; }

    // Expose protected helpers for testing
    bool doTransmit(const uint8_t* data, size_t len) { return transmit<I2CInterface>(data, len); }
    bool doReceive(uint8_t* buf, size_t len)          { return receive<I2CInterface>(buf, len); }
    void callDeferMs(uint32_t ms) { defer_ms_(ms); }
    void callDeferUs(uint32_t us) { defer_us_(us); }
...
public:
    DualChip(I2CInterface& i2c, SPIInterface& spi)
        : Chip<I2CInterface, SPIInterface>(i2c, spi) {}

    bool initialize() override { return true; }
    bool reset()      override { return true; }
    bool isReady() const override { return true; }
    ChipBase::Status getStatus() const override { return ChipBase::Status::Ready; }
    std::string getDeviceId() const override { return "DualChip"; }
...
TEST_F(ChipTest, SetDeferCallbacksAreStoredAndCallable) {
    ...
    chip.setDeferCallbacks(
        [&](uint32_t) { defer_ms_called = true; },
        [&](uint32_t) { defer_us_called = true; }
    );
...
TEST_F(DualChipTest, GetCommInterfacesReturnsAllTwoInterfaces) {
    ...
    for (auto* iface : interfaces) {
        if (iface == &i2c) has_i2c = true;
        if (iface == &spi) has_spi = true;
    }
    EXPECT_TRUE(has_i2c);
    EXPECT_TRUE(has_spi);
}
